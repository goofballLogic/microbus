...
// log messages
mb.Bus.prototype.log = function( callback ) {

	this.loggers.push( callback );
...
mb.Bus.prototype.sendFrom = function() {
	...
	function logMessageSent( logger ) { logger( "send", sender, args ); }
	function logMessageReceipt( receiver, logger ) { logger( "receive", receiver.owner, sender, args ); }
	loggers.forEach( logMessageSent );
	...
	function sendToReceiver( receiver ) {
		...
		loggers.forEach( logMessageReceipt.bind( null, receiver ) );
